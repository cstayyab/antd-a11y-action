...
function ruleDescriptor(rule: RuleInfo, failOn: Impact) {
  const blocking = impactRank(rule.impact) >= impactRank(failOn);
  return {
    id: rule.id,
    name: rule.id,
...
function resultFor(finding: Finding, ruleIndex: number) {
  return {
...
export function toSarif(result: ScanResult, failOn: Impact, version: string) {
  const rules = [...result.rules.values()].sort((a, b) => a.id.localeCompare(b.id));
    ...
    runs: [
      {
        ...
        automationDetails: { id: `${TOOL_NAME}/` },
        results: result.findings.map((finding) => resultFor(finding, index.get(finding.ruleId)!)),
      },
    ],
  };
}
