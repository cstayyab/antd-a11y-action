// Impact and blocking: how serious a finding is, and whether it fails the check. Shared by the
// action and by the plugin's processor, so a local ESLint run blocks exactly when the PR check would.
import jsxA11y from 'eslint-plugin-jsx-a11y';
import type { AntdA11yDocs, Impact } from '../utils/create-rule.js';
import { rules as antdRules } from '../plugin.js';
...
function antdDocs(ruleId: string): (Partial<AntdA11yDocs> & { url?: string }) | undefined {
  const rule = (antdRules as unknown as Record<string, RuleMeta>)[ruleId.slice(ANTD_PREFIX.length)];
